...
| `IMGUR_CLIENT_ID` | [Imgur API](https://api.imgur.com/oauth2/addclient) の **Client ID**（匿名アップロード用。LINE は **HTTPS の画像 URL** が必須のため） |
...
Messaging API の `image` 型は **`originalContentUrl` / `previewImageUrl` に到達可能な HTTPS URL** が必要です。  
リポジトリ内の PNG をそのまま送ることはできないため、**Imgur 匿名アップロード**で公開 URL を取得しています。  
自社 CDN や GCS 等に置き換える場合は `line.mjs` の前段を差し替えてください。
